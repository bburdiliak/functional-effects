...
object ErrorShortCircuit extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.orElse(ZIO.succeed(ExitCode.success)).exitCode
}
...
object ErrorRecoveryCatchAll extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    failed.catchAll((s: String) => putStrLn(s)).exitCode
}
...
object AlarmApp extends App {
  ...
  lazy val getAlarmDuration: ZIO[Console, IOException, Duration] = {
    def parseDuration(input: String): IO[NumberFormatException, Duration] =
      for {
        number <- ZIO.effect(input.toLong).refineOrDie { case e: NumberFormatException => e }
      } yield Duration(number, TimeUnit.SECONDS)

    def fallback(input: String): ZIO[Console, IOException, Duration] =
      putStrLn(s"$input is not a valid number.") *> getAlarmDuration
...
object Sandbox extends App {
   ...
   */
  def run(args: List[String]): ZIO[ZEnv, Nothing, ExitCode] =
    composed.sandbox.mapError(cause => putStrLn(cause.prettyPrint)).ignore.exitCode
}
